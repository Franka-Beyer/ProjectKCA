...
async def dispatch_openai_requests(
    ...
    client.additional_kwargs={"temperature":temperature, "max_new_tokens":max_tokens}
    ...
    #await asyncio.gather(*async_responses)
    await client.close() #closing event loop to avoid error
...
def get_completion(messages_list: list, model: str, temperature: float = 0.0):
    for i in range(MAX_API_RETRY):
        ...
        except Exception as e1: #modified to avoid error
            print("Error: Invalid Request: ", e1)
            return None
    ...
    if prompt_type == "hallucination_judge":
        ...
        if len(parts) == 3 or len(parts) >= 3:
            ...
            try:
                ...
                span = []
            #explain = parts[2]
            explain = str(parts[-1:]) #modifying to allow for more parts; trying to deal with formatting error
        else:
            pattern = r"^(\d+)[ \n]+(\[.*\])[ \n]+(.*)$"
...
def get_single_score(input_file, testset_file, output_file, prompt_file, quantitative_analysis, analysis_file, prompt_type=None, use_demo=False,
                     model="qwen2:1.5b", temperature=0.0, batch_size=1, no_sorry=False):
    input_examples = get_json_list(input_file)
    testset_examples = json.loads(open(os.path.expanduser(testset_file), "r").read())
    ...
    for prompt in get_json_list(prompt_file):
        if prompt["prompt_type"] == prompt_type:
            ...
            break
    #adding option for manual analysis
    ...
    for i in tqdm.tqdm(range(len(curr_result), len(review_examples), batch_size)):
        ...
        for example in examples:
            ...
            if quantitative_analysis: #adding option for manual analysis
        ...
        if not quantitative_analysis: 
            completions = get_completion(messages_list, model, temperature) #modified
            
        if completions:
            try:
                #results = [completion['choices'][0]['message']['content'] for completion in completions] #modified for new modules
...
def main():
    ...
    parser.add_argument("--answer_file", type=str, default="~/Downloads/Projekt/KCA/evaluation_results/answer_greedy/data-qwen2-1.5b_shot-5_wizardlm_testset_greedy.jsonl")
    parser.add_argument("--testset_file", type=str, default="~/Downloads/Projekt/KCA/data/processed_results/qwen2-1.5b_shot-5_wizardlm_testset_sorry.json")
    parser.add_argument("--review_file", type=str, default="~/Downloads/Projekt/KCA/evaluation_results/review_greedy/data_TinyLLM_shot-5_lima_testset_hallucination_greedy.jsonl")
    parser.add_argument("--prompt_file", type=str, default="~/Downloads/Projekt/KCA/eval/gpt_judge/gpt_judge_prompt.jsonl")
    parser.add_argument("--prompt_type", type=str, default="hallucination_judge")
    parser.add_argument("--use_demo", default=True, action="store_true")
    parser.add_argument("--review_model", type=str, default="qwen2:1.5b")
    parser.add_argument("--batch_size", type=int, default=3)
    parser.add_argument("--check_error_parse", action="store_true")
    parser.add_argument("--no_sorry", action="store_true")
    #adding option to output the prompts to the judge model as a file (for human judgement)
    parser.add_argument("--qualitative_analysis", action="store_true", default=True)
    parser.add_argument("--analysis_file", type=str, default="~/Downloads/Projekt/KCA/qualitative_analysis/qualitative_analysis_qwen2-1.5b_wizardlm_testset_greedy.json")
    ...
    if not args.check_error_parse:
        get_single_score(args.answer_file,
                         ...
                         args.review_file,
                         args.prompt_file,
                         args.qualitative_analysis, #adding new arguments
                         args.analysis_file,
