...
def run_eval(model_path, model_id, conv_temp, question_file, answer_file, num_gpus, do_sample=False):
    if question_file.endswith(".jsonl"):
        ...
        ques_jsons = json.loads(open(os.path.expanduser(question_file), "r").read()) #modified path
...
def get_model_answers(model_path, model_id, conv_temp, question_jsons, do_sample=False):
    if "Tiny-LLM" in model_path: #modified to allow for ollama alternative
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, default="qwen2:1.5b", required=False) #qwen2:1.5b, ~/Downloads/Projekt/Tiny-LLM-llama
    parser.add_argument("--model-id", type=str, default="qwen2:1.5b", required=False) #or: Tiny-LLM-llama
    parser.add_argument("--conv-temp", type=str, default="vicuna")
    parser.add_argument("--question-file", type=str, default="~/Downloads/Projekt/KCA/data/processed_results/qwen2-1.5b_shot-5_truthfulqa_test_truthset_sorry.json", required=False)
    parser.add_argument("--answer-file", type=str, default="~/Downloads/Projekt/KCA/evaluation_results/answer_greedy/data-qwen2-1.5b_shot-5_truthfulqa_test_truthset_greedy.jsonl")
