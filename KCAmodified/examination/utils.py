...
from examination.dispatch_openai_requests import dispatch_openai_chat_requesets, dispatch_openai_prompt_requesets
...
def get_next_word_predictions(
    ...
    disable_tqdm=False
) -> Tuple[List[Union[int, str]], List[List[float]]]:
    predictions, probs = [], []
    if not disable_tqdm:
...
def load_hf_lm_and_tokenizer(
    ...
    #trying to fix error with repo id
    from huggingface_hub import Repository
    
    #adding in the path expansion
    #model_name_or_path = os.path.expanduser(model_name_or_path)
    #rep = Repository(local_dir=model_name_or_path, repo_type="model")
    #model_name_or_path = rep
...
def query_openai_chat_model(engine, instances, output_path=None, batch_size=10, retry_limit=5, reuse_existing_outputs=False, **completion_kwargs): #make sure to think again
    '''
    ...
    for i in range(0, len(instances), batch_size):
        ...
        while retry_count < retry_limit:
            try:
                if batch_size > 1:
                    ...
                    client.additional_kwargs={"enable_thinking":False, "top_k":20, "top_p":0.8, "min_p":0}
                    outputs = [client.chat.completions.create( #modified for new module
                    #outputs = [openai.ChatCompletion.create(
                              model=engine,
                              messages=messages_list[0],
                              **completion_kwargs, #uncommented to limit output tokens
                    )]
                retry_count = 0
                break
            except Exception as e:
        ...
        for instance, output in zip(batch, outputs):
            ...
            instance[f"output"] = output.choices[0].message.content #adjusted to new module
            #instance["response_metadata"] = output #commented out to avoid serialization error
...
def query_openai_model(engine, instances, output_path=None, batch_size=10, retry_limit=5, reuse_existing_outputs=True, **completion_kwargs):
    ...
    for i in range(0, len(instances), batch_size):
        ...
        for instance, output in zip(batch, outputs):
            ...
            instance[f"output"] = output.choices[0].text #for new API
            #instance["response_metadata"] = output #commented out to avoid error with non-serializable object, is not needed
            results.append(instance)
            if output_path is not None:
                fout.write(json.dumps(instance) + "\n")
                fout.flush()
